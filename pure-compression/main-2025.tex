...
Furthermore, we can assume that $\beta_i$ is non-increasing.
...
\]
\begin{conjecture}
The above maximization has value equal to the sum of the $d$ largest eigenvalues of 
$\trace_{R}\ket{\rho}^{RA}$, or $\sum_{k=1}^d \beta_k$
\end{conjecture}
...
We have then upper bounded the original maximization to the following:
\begin{equation*}
\begin{array}{rl}
    \text{maximize}    & \left(\sum_i \beta_i \alpha_i\right)^2 \\
    \text{subject to:} & \sum_i \alpha_i^2 \le d \\
                       & \alpha_i \in [0,1]
\end{array}
\end{equation*}

By applying Cauchy Schwarz to the above upper bounds the objective by:
\begin{align*}
    \left(\langle \sum_i \beta_i \alpha_i \rangle\right)^2
        &= \left(\sum_i \sqrt{\beta_i} (\sqrt{\beta_i}\alpha_i)\right) \\
    &\le \left(\sum_i \beta_i\right)\left(\beta_i\alpha_i^2\right)
    &= \left(\beta_i\alpha_i^2\right)
\end{align*}
Since we have that $\norm{\beta_i}_1 = 1$ by assumption.

Furthermore, since $\cdot^2$ is bijective on $[0,1]$ we change $\alpha_i^2$ to $\gamma_i$
to obtain the following maximization problem:
\begin{equation*}
\begin{array}{rl}
    \text{maximize}    & \sum_i \beta_i\gamma_i \\
    \text{subject to:} & \sum_i \gamma_i \le d \\
                       & \gamma_i \in [0,1]
\end{array}
\end{equation*}
It is clear that the maximization is equal to the sum of the largest $d$ values of $\boldsymbol\beta$,
attained by taking $\gamma_i$ to be $[i \le d]$.

% Note that the max value is attained by the projective channel described in Nick's thesis,
% we have given an upper bound but lower bound is omitted
